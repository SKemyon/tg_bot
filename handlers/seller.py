...
async def start_selling(message: Message, state: FSMContext):
    await message.answer(
        ...
        "4. Комиссия в 1% взимается только в случае успешной продажи\n\n"
        "5. Использование Telegram-бота означает согласие с условиями Оферты (смотреть в разделе Правила).\n\n\n"
...
async def accept_deal(callback: types.CallbackQuery):
    ...
    await callback.bot.send_message(settings.moderator_chat_id, f"{lot_title} за {bid}:Контакт {seller_contact} передан победителю {winner_id}.")
